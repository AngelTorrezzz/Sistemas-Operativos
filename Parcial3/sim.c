...
/*Integrantes: Angel de Jesus Alvarado Torres y Jose Guadalupe Robles Jimenez*/
...
int main(){
    ...
    while(1){
        ...
        if(opcion == 1){
            pid= min + rand() % (max - min + 1);
            printf("Tamaño del proceso en Kb: ");
            scanf("%f",&tamproc);
            tamproc = (tamproc / 4); // se divide entre 4 porque es el tamaño de kb que contiene cada pagina
            tamproc = ceil(tamproc);
            num_paginas = (int)tamproc;
            if(num_paginas > 8){
                printf("el tamaño del proceso excede la cantidad de paginas\n");
            }else{
